...
class CafKernel(Kernel):
    ...
    def compile_with_caf_wrapper(self, source_filename, binary_filename, fcflags=None, ldflags=None):
        return self.create_jupyter_subprocess(['caf', source_filename] +
                                              fcflags +
                                              ['-o', binary_filename] +
                                              ldflags)
            ...
            with self.new_temp_file(suffix='.out') as binary_file:
                ...
                if p.returncode != 0:  # Compilation failed
                    ...
                    return {'status': 'ok', 'execution_count': self.execution_count, 'payload': [],
                            'user_expressions': {}}
        p = self.create_jupyter_subprocess(['cafrun', '-np', str(magics['num_images']).strip(), binary_file.name] + magics['args'])
        while p.poll() is None:
            p.write_contents()
        p.write_contents()
